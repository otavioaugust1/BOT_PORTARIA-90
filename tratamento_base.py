...
tempo_inicial = (
    time.time()
)   # tempo inicial para calcular o tempo de execução do código

from datetime import date  # Biblioteca para manipular datas
from datetime import timedelta  # Biblioteca para manipular datas
...
from selenium.webdriver.common.by import (
    By,
)  # Biblioteca para automatizar o navegador
...
print(
    f"[OK] Início do processo:=====================================================>: {time.strftime('%H:%M:%S')}"
)
...
data_passada = (date.today() - relativedelta(months=1)).strftime(
    '%Y%m'
)   # Data atual menos 1 mês no formato YYYYMM
data_passada_a = (date.today() - relativedelta(months=2)).strftime(
    '%Y%m'
)   # Data atual menos 2 mês no formato YYYYMM
...
print(
    f"[OK] Criando as pastas ======================================================>: {time.strftime('%H:%M:%S')}"
)
...
try:
    with urllib.request.urlopen(url_c) as response, open(
        filename_c, 'wb'
    ) as out_file:
...
print(
    f"[OK] Download dos arquivos CNES =============================================>: {time.strftime('%H:%M:%S')}"
)
...
arquivo = [
    a
    for a in arquivos
    if a.startswith(f'TabelaUnificada_{data_corrente}') and a.endswith('.zip')
]
arquivo_a = [
    a
    for a in arquivos
    if a.startswith(f'TabelaUnificada_{data_passada}') and a.endswith('.zip')
]

if arquivo:
    url_s = (
        f'ftp://ftp2.datasus.gov.br/pub/sistemas/tup/downloads/{arquivo[0]}'
    )
    filename_s = pasta_entrada + f'BASE_DE_DADOS_SIGTAP_{data_corrente}.ZIP'

    with urllib.request.urlopen(url_s) as response, open(
        filename_s, 'wb'
    ) as out_file:
        data = response.read()  # Lê os dados do arquivo
        out_file.write(data)  # Escreve os dados em um arquivo local
    ...
    print(
        f"[OK] Download dos arquivos SIGTAP ===========================================>: {time.strftime('%H:%M:%S')}"
    )
else:
    url_s = (
        f'ftp://ftp2.datasus.gov.br/pub/sistemas/tup/downloads/{arquivo_a[0]}'
    )
    ...
    with urllib.request.urlopen(url_s) as response, open(
        ...
        out_file.write(data)  # Escreve os dados em um arquivo local
    print(
        f"[OK] Download dos arquivos SIGTAP - MES ANTERIOR ============================>: {time.strftime('%H:%M:%S')}"
    )
...
"""
print(
    f"[OK] Download do arquivo BASE_DE_DADOS_SAIPS.xlsx ===========================>: {time.strftime('%H:%M:%S')}"
)
...
print(
    f"[OK] Download do arquivo BASE_DE_DADOS_TETO_MAC.xlsx ========================>: {time.strftime('%H:%M:%S')}"
)
...
dado_cnes = glob('BASE/BASE_DE_DADOS_CNES*.ZIP')[
    0
...
with zipfile.ZipFile(dado_cnes, 'r') as myzip:
    ...
    df_cnes = next(
        (
            file
            for file in files
            if re.match(r'tbEstabelecimento.*\.csv', file)
        ),
        None,
    )   # Encontrando o arquivo CSV com a expressão regular
    ...
    if df_cnes:
        with myzip.open(df_cnes) as myfile:
            ...
            print(
                f"[OK] Importação do CNES:=====================================================>: {time.strftime('%H:%M:%S')}"
            )
    else:
        print(
            f"[ERRO] Importação do CNES - Arquivo CSV não encontrado ======================>: {time.strftime('%H:%M:%S')}"
        )
...
with zipfile.ZipFile(dado_cnes, 'r') as myzip:
    files = myzip.namelist()   # Obtendo a lista de arquivos no ZIP
    df_cnes_servicos = next(
        (file for file in files if re.match(r'rlEstabServClass.*\.csv', file)),
        None,
    )   # Encontrando o arquivo CSV com a expressão regular
    ...
    if df_cnes_servicos:
        with myzip.open(df_cnes_servicos) as myfile:
            ...
            print(
                f"[OK] Importação CNES - Serviço x Classificação ==============================>: {time.strftime('%H:%M:%S')}"
            )
    else:
        print(
            f"[ERRO] Importação CNES - Serviço x Classificação [Arquivo não encontrado] ===>: {time.strftime('%H:%M:%S')}"
        )
...
with zipfile.ZipFile(dado_cnes, 'r') as myzip:
    files = myzip.namelist()   # Obtendo a lista de arquivos no ZIP
    df_cnes_habilitacao = next(
        (file for file in files if re.match(r'rlEstabSipac.*\.csv', file)),
        None,
    )   # Encontrando o arquivo CSV com a expressão regular
    ...
    if df_cnes_habilitacao:
        with myzip.open(df_cnes_habilitacao) as myfile:
            ...
            print(
                f"[OK] Importação CNES - Habilitação ==========================================>: {time.strftime('%H:%M:%S')}"
            )
    else:
        print(
            f'[ERRO] Importação CNES - Habilitação - [Arquivo CSV não encontrado] =========>: {time.strftime("%H:%M:%S")}'
        )
...
with zipfile.ZipFile(dado_cnes, 'r') as myzip:
    files = myzip.namelist()   # Obtendo a lista de arquivos no ZIP
    df_cnes_habilitacao_desc = next(
        (
            file
            for file in files
            if re.match(r'tbSubGruposHabilitacao.*\.csv', file)
        ),
        None,
    )   # Encontrando o arquivo CSV com a expressão regular
    ...
    if df_cnes_habilitacao_desc:
        with myzip.open(df_cnes_habilitacao_desc) as myfile:
            df_cnes_habilitacao_desc = pd.read_csv(
                myfile,
                sep=';',
                encoding='latin-1',
                low_memory=False,
            ...
            print(
                f"[OK] Importação CNES - Habilitação e Descrição ==============================>: {time.strftime('%H:%M:%S')}"
            )
    else:
        print(
            f'[ERRO] Importação CNES - Habilitação e Descrição - [Arquivo não encontrado] =>: {time.strftime("%H:%M:%S")}'
        )

df_cnes_habilitacao = pd.merge(
    df_cnes_habilitacao,
    df_cnes_habilitacao_desc,
    left_on='COD_SUB_GRUPO_HABILITACAO',
    right_on='CO_CODIGO_GRUPO',
    how='left',
)
...
with zipfile.ZipFile(dado_cnes, 'r') as myzip:
    ...
    df_cnes_leitos = next(
        (
            file
            for file in files
            if re.match(r'rlEstabComplementar.*\.csv', file)
        ),
        None,
    )   # Encontrando o arquivo CSV com a expressão regular
    ...
    if df_cnes_leitos:
        with myzip.open(df_cnes_leitos) as myfile:
            ...
            print(
                f"[OK] Importação CNES - Leitos, leitos SUS ===================================>: {time.strftime('%H:%M:%S')}"
            )
    else:
        print(
            f'[ERRO] Importação CNES - Leitos, leitos SUS - [Arquivo não encontrado] ======>: {time.strftime("%H:%M:%S")}'
        )


## Merge dos dados do CNES
df_base_cnes_s = pd.merge(
    df_cnes, df_cnes_servicos, on='CO_UNIDADE', how='left'
)
df_base_cnes_h = pd.merge(
    df_cnes, df_cnes_habilitacao, on='CO_UNIDADE', how='left'
)
df_base_cnes_l = pd.merge(df_cnes, df_cnes_leitos, on='CO_UNIDADE', how='left')
print(
    f"[OK] Merge dos dados do CNES ================================================>: {time.strftime('%H:%M:%S')}"
)


## Exportando os dados
df_base_cnes_s.to_csv(
    'BASE/BASE_CNES_SERVICOS.csv', sep=';', encoding='latin-1', index=False
)   # Exportando o arquivo para csv
df_base_cnes_h.to_csv(
    'BASE/BASE_CNES_HABILITACAO.csv', sep=';', encoding='latin-1', index=False
)   # Exportando o arquivo para csv
df_base_cnes_l.to_csv(
    'BASE/BASE_CNES_LEITOS.csv', sep=';', encoding='latin-1', index=False
)   # Exportando o arquivo para csv

print(
    f"[OK] Exportando os dados do CNES ============================================>: {time.strftime('%H:%M:%S')}"
)
## Fim do código
...
df_teto_mac = pd.read_excel(
    dado_tetp, sheet_name='relatorioTetoFinanceiroBrasilEx'
)
df_teto_mac.drop(
    df_teto_mac.loc[df_teto_mac['Descrição Gestão'] == 'Total UF'].index,
    inplace=True,
)
print(
    f"[OK] Importação Teto MAC  ===================================================>: {time.strftime('%H:%M:%S')}"
)

## Exportando os dados
df_teto_mac.to_csv(
    f'{pasta_entrada}/BASE_TETO_MAC.csv',
    sep=';',
    encoding='latin-1',
    index=False,
)
...
with zipfile.ZipFile(dado_sigtap) as myzip:
    with myzip.open('tb_procedimento.txt') as myfile:
        df_sigtap = pd.read_fwf(
            myfile,
            colspecs=[(0, 10), (10, 260), (260, 330)],
            names=['CO_PROCEDIMENTO', 'NO_PROCEDIMENTO', 'DT_COMPETENCIA'],
            encoding='latin',
        )
df_sigtap.drop(['DT_COMPETENCIA'], axis=1, inplace=True)
print(
    f"[OK] Importação SIGTAP ======================================================>: {time.strftime('%H:%M:%S')}"
)
...
with zipfile.ZipFile(dado_sigtap) as myzip:   # Abrindo o arquivo zip
    with myzip.open('rl_procedimento_servico.txt') as myfile:
        df_sigtap_servico = pd.read_fwf(
            myfile,
            colspecs=[(0, 10), (10, 13), (13, 16), (16, 22)],
            names=[
                'CO_PROCEDIMENTO',
                'CO_SERVICO',
                'CO_CLASSIFICACAO',
...
df_sigtap_servico.drop(
    ['DT_COMPETENCIA'], axis=1, inplace=True
)   # Removendo colunas desnecessárias
print(
    f"[OK] Importação SIGTAP - Serviços ===========================================>: {time.strftime('%H:%M:%S')}"
)
...
with zipfile.ZipFile(dado_sigtap) as myzip:   # Abrindo o arquivo zip
    with myzip.open('rl_procedimento_habilitacao.txt') as myfile:
        df_sigtap_habilitacao = pd.read_fwf(
            myfile,
            colspecs=[(0, 10), (10, 14), (14, 18), (18, 24)],
            names=[
                'CO_PROCEDIMENTO',
                'CO_HABILITACAO',
...
df_sigtap_habilitacao.drop(
    ['NU_GRUPO_HABILITACAO', 'DT_COMPETENCIA'], axis=1, inplace=True
)   # Removendo colunas desnecessárias
print(
    f"[OK] Importação SIGTAP - Habilitação ========================================>: {time.strftime('%H:%M:%S')}"
)
...
with zipfile.ZipFile(dado_sigtap) as myzip:   # Abrindo o arquivo zip
    with myzip.open('rl_procedimento_registro.txt') as myfile:
        df_sigtap_modalidade = pd.read_fwf(
            myfile,
            colspecs=[(0, 10), (10, 12), (12, 18)],
            names=['CO_PROCEDIMENTO', 'CO_REGISTRO', 'DT_COMPETENCIA'],
        )   # Lendo o arquivo txt
df_sigtap_modalidade.drop(
    ['DT_COMPETENCIA'], axis=1, inplace=True
)   # Removendo colunas desnecessárias

with zipfile.ZipFile(dado_sigtap) as myzip:   # Abrindo o arquivo zip
    with myzip.open('tb_registro.txt') as myfile:
        df_sigtap_registro = pd.read_fwf(
            myfile,
            colspecs=[(0, 2), (2, 52), (52, 58)],
            names=['CO_REGISTRO', 'NO_REGISTRO', 'DT_COMPETENCIA'],
            encoding='latin',
        )   # Lendo o arquivo txt
df_sigtap_registro.drop(
    ['DT_COMPETENCIA'], axis=1, inplace=True
)   # Removendo colunas desnecessárias
df_sigtap_modalidade = pd.merge(
    df_sigtap_modalidade, df_sigtap_registro, on='CO_REGISTRO', how='left'
)   # Juntando os arquivos
print(
    f"[OK] Importação SIGTAP - Forma de Registro ==================================>: {time.strftime('%H:%M:%S')}"
)


## SIGTAP - PLANILHA
df_planilha_proc = glob('BASE-T/BASE_DE_DADOS_SAIPS*.xlsx')[
    0
]   # Listando os arquivos do diretório
df_planilha_proc = pd.read_excel(
    df_planilha_proc, sheet_name='PROCEDIMENTOS'
)   # Lendo o arquivo excel
df_planilha_proc.rename(
    columns={
        'Código do Procedimento': 'CO_PROCEDIMENTO',
        'Unnamed: 1': 'SUBGRUPO',
        'Unnamed: 2': 'COD_PROCEDIMENTO',
        'Unnamed: 3': 'DEC_PROCEDIMENTO',
        'Unnamed: 4': 'codigo',
        'Dados de Produção Brasil (2018 a 2022)': 'QT_PROD_SIASUS',
        'Unnamed: 6': '% ELET SIASUS',
        'Unnamed: 7': 'QT_PROD AIH 2018 a 2022',
        'Unnamed: 8': '% ELET AIH',
        'Exigência SIGTAP': 'EXIGE SERVIÇO',
        'Unnamed: 10': 'EXIGE HABILITACAO',
        'Instrumento de REGISTRO': 'BPA_I',
        'Unnamed: 12': 'AIH',
        'Unnamed: 13': 'APAC',
    },
    inplace=True,
...
df_planilha_proc.drop(
    ['codigo'], axis=1, inplace=True
)   # Remove a coluna codigo


## Juntando os arquivos - SIGTAP
df_sigtap_f = pd.merge(
    df_planilha_proc,
    df_sigtap_modalidade,
    left_on='COD_PROCEDIMENTO',
    right_on='CO_PROCEDIMENTO',
    how='left',
)   # Juntando os arquivos 3
df_sigtap_f = pd.merge(
    df_sigtap_f, df_sigtap_servico, on='CO_PROCEDIMENTO', how='left'
)   # Juntando os arquivos
df_sigtap_f = pd.merge(
    df_sigtap_f, df_sigtap_habilitacao, on='CO_PROCEDIMENTO', how='left'
)   # Juntando os arquivos
df_sigtap_f.drop(
    ['CO_PROCEDIMENTO'], axis=1, inplace=True
)   # Removendo colunas desnecessárias
print(
    f"[OK] Juntando os arquivos - SIGTAP:==========================================>: {time.strftime('%H:%M:%S')}"
)

## Exportando o arquivo
df_sigtap_f.to_csv(
    'BASE/BASE_SIGTAP_GERAL.csv', sep=';', encoding='latin-1', index=False
)   # Exportando o arquivo para csv
print(
    f"[OK] Exportando o arquivo - SIGTAP ==========================================>: {time.strftime('%H:%M:%S')}"
)

## Fechando os arquivos
myfile.close()
print(
    f"[OK] Fim do código  =========================================================>: {time.strftime('%H:%M:%S')}"
)
